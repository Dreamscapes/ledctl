...
/**
 * @summary   LED Controller
 *
 * @class
 * @param     {String}    identifier  The LED identifier - one of the values returned by
 *                                    {@link LEDController.discover}
 */
function LEDController (identifier) {

  // DISCLAIMER: This is probably the longest constructor I have ever written. Not that I have
  // written many constructors... I will consider splitting this into individual functions, just for
  // the fun of it. Someday, maybe.
  ...
  if (! _(LEDController.discover()).contains(identifier)) {
    throw new Error(fmt('No such LED: %s in %s', identifier, LEDController.ROOT))
  }

  /**
   * @summary   The LED's ID, as given to the constructor.
   *
   * @desc      Just a helper for you, dear developer - I don't really need this in the source...
   *
   * @readonly
  ...
  brightness =
  { min: 0
  , max: parseInt(contentOf(this.location, 'max_brightness'))
  }
  // brightness.cur
  ...
  triggers =
  { all: getTriggerData(this.location).all
  }
  // triggers.cur
 ...
 *            second, and the rate is 1, then the actual duration of the blink event will be
 *            calculated as duration times rate, i.e. 1s x 1 => 1 second. The rate gives you an
 ...
 * @param     {Function}    done      A callback that you should call when you are done parsing -
 *                                    as is common, the first argument should either be an Error
 *                                    object or null, the array of {@link LEDController.Blink}
 *                                    events should come second.
 ...
 *
 * @param  {String} parser  The name of the parser - method of this name will be added to prototype
 * @param  {LEDController.Parser} handler The actual parser.
 */
LEDController.register = function register (parser, handler) {
  ...
  if (LEDController.prototype[parser]) {
    throw new Error(fmt('Function: %s already defined on prototype', parser))
  }
  ...
  LEDController.prototype[parser] = function customParser (input, done) {
    ...
    handler(input, function (err, data) {
      // Normalise
      ...
      data.forEach(function (blink, i) {
        if (data[i + 1]) {
          that.blinker.push(blink, normaliseCallback())   // Let's leave the callback empty!
        } else {
...
LEDController.prototype.trigger = function trigger (value, done) {
  ...
  if (! _(this.triggers.all).contains(value)) {
    // It's not! You will witness your mistake... in the next tick!
    process.nextTick(function () {
      done(new Error(fmt('Unsupported trigger: \'%s\' for LED %s', value, that.id)))
    })
  } else {
...
LEDController.prototype.turnOn = function turnOn (done) {
  ...
  this.writer.push({ to: 'brightness', data: this.brightness.max }, done)
...
LEDController.prototype.turnOff = function turnOff (done) {
  ...
  this.writer.push({ to: 'brightness', data: this.brightness.min }, done)
...
function blinkWorker (opts, done) {
  ...
  if (timeOn === 0) {
    ...
    return setTimeout(done, timeOff)
  }
  ...
  this.turnOn(function (err) {
    ...
    if (err) {
      return done(err)    // The story ends here
    }

    setTimeout(function () {
      // This timeout keeps the LED on
      that.turnOff(function () {
        setTimeout(function () {
          // This timeout keeps the LED off for the remaining duration of the sequence
          done()
        }, timeOff)
      })
    }, timeOn)
